...
public class EditSubjectViewModel : ViewModelBase, IDataErrorInfo
{
    ...
    public ObservableCollection<Subject> AvailablePrerequisites
    {
        get
        {
            ...
            {
                _availablePrerequisites = LoadSubjects();
    ...
    private ObservableCollection<FacultyMember>? _availableFacultyMembers = null;
    ...
    private void AddFacultyMember(object? obj)
    {
        ...
        {
            if (AssignedFacultyMembers is not null && !AssignedFacultyMembers.Contains(facultyMember))
            {
    ...
    private void RemoveFacultyMember(object? obj)
    {
        if (obj is FacultyMember facultyMember)
        {
            if (AssignedFacultyMembers is not null)
            {
    ...
    private void SaveData(object? obj)
    {
        ...
        _subject.CourseCode = CourseCode;
        _subject.Title = Title;
        _subject.Instructor = Instructor;
        _subject.Schedule = Schedule;
        _subject.Description = Description;
        _subject.Credits = Credits;
        _subject.Department = Department;
        _subject.Prerequisite = AvailablePrerequisites.Where(s => s.IsSelected).ToList();
        _subject.Students = AssignedStudents;
        _subject.FacultyMembers = AssignedFacultyMembers;
    ...
    private void LoadSubjectData()
    {
        ...
        if (subjects is not null)
        {
            ...
            this.CourseCode = _subject.CourseCode;
            this.Title = _subject.Title;
            this.Instructor = _subject.Instructor;
            this.Schedule = _subject.Schedule;
            this.Description = _subject.Description;
            if (_subject.Credits.HasValue)
            {
                this.Credits = _subject.Credits.Value;
            }
            else
            {
                this.Credits = 0;
            ...
            this.Department = _subject.Department;
            if (_subject.Prerequisite is null)
            {
                return;
            }
            foreach (Subject Prerequisite in _subject.Prerequisite)
            {
                if (Prerequisite is not null && AvailablePrerequisites is not null)
                {
                    var availablePrerequisites = AvailablePrerequisites
                        .FirstOrDefault(s => s.SubjectId == Prerequisite.SubjectId);
            ...
            if (_subject.FacultyMembers is not null)
            {
                this.AssignedFacultyMembers =
                    new ObservableCollection<FacultyMember>(_subject.FacultyMembers);
            }
        }
    }
}
